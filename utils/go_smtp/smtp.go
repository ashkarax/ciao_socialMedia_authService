...
func NewSmtpUtils(smtpConfigs *config_authSvc.Smtp) interface_smtp_authSvc.ISmtp {
	return &SmtpCredentials{SmtpConfig: smtpConfigs}
}
func (smtpCredentials *SmtpCredentials) SendVerificationEmailWithOtp(otp int, recieverEmail string, recieverName string) error {
...
func (smtpCredentials *SmtpCredentials) SendRestPasswordEmailOtp(otp int, recieverEmail string) error {
	from := smtpCredentials.SmtpConfig.SmtpSender
	password := smtpCredentials.SmtpConfig.SmtpPassword
	to := []string{recieverEmail}
	smtpHost := smtpCredentials.SmtpConfig.SmtpHost
